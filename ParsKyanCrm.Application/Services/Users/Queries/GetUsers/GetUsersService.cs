...
namespace ParsKyanCrm.Application.Services.Users.Queries.GetUsers
{
    ...
    public class GetUsersService : IGetUsersService
    {
        ...
        public async Task<UserRolesDto> Execute(int? id = null)
        {
            try
            {
                ...
                if (id != null && id != 0)
                {
                    var q_Find = await _context.UserRoles.Include(p => p.User).Include(p => p.Role).FirstOrDefaultAsync(p => p.UserID == int.Parse(id.ToString()));
