        ...
        AjaxCallAction("POST", "/api/admin/Customers/Get_Customerss", JSON.stringify({ Search: $("#txtSearch").val(), PageIndex: 1, PageSize: $("#cboSelectCount").val() }), true, function (res) {
            ...
            if (res.isSuccess) {
                ...
                for (var i = 0; i < res.data.length; i++) {
                    ...
                    strM += "<tr><td>" + (i + 1) + "</td><td>" + res.data[i].agentName + "</td><td>" + res.data[i].agentMobile + "</td><td>" + res.data[i].tel + "</td><td><a title='ویرایش' href='/Admin/Customers/EditCustomers?id=" + res.data[i].customerId + "' class='btn btn-edit fontForAllPage'><i class='fa fa-edit'></i></a></td></tr>";
