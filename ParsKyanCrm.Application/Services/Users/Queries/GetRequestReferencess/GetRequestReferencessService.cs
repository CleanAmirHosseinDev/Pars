...
namespace ParsKyanCrm.Application.Services.Users.Queries.GetRequestReferencess
{
    public class GetRequestReferencessService : IGetRequestReferencessService
    {
        ...
        public GetRequestReferencessService(IDataBaseContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;            
        }
...
(select RoleDesc from UserRoles  as ur inner join Roles as r on ur.RoleID=r.RoleID where UserID=rr.ReciveUser)RoleDesReciver,
(select RealName from Users  as u  where UserID=rr.ReciveUser)ReciverName,
rr.[LevelStepSettingIndexID]
