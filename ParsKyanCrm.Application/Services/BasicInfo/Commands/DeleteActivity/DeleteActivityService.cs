...
namespace ParsKyanCrm.Application.Services.BasicInfo.Commands.DeleteActivity
{
    ...
    public class DeleteActivityService : IDeleteActivityService
    {
        ...
        public DeleteActivityService(IDataBaseContext context, IMapper mapper, IWebHostEnvironment env)
        {
            _context = context;
            _mapper = mapper;
            _env = env;
        ...
        public ResultDto Execute(int id)
        {
            ...
            try
            {
                ...
                Ado_NetOperation.SqlUpdate(typeof(Domain.Entities.Activity).Name, new Dictionary<string, object>()
                    ...
                    }, string.Format("ActivityId" + " = '{0}' ", id));

                return new ResultDto()
                {
                    IsSuccess = true,
                    Message = "فعالیت شرکت موردنظر با موفقیت حذف شد"
                };
            }
            catch (Exception ex)
            {
